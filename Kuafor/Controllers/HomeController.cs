...
namespace Kuafor.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult KuaförList()
        {
            List<Kuaför> KuaförList = db.Kuaför.ToList();
            var kuaförliste = from kf in KuaförList
            ...
            return View(kuaförliste.ToList());
        }
        [HttpPost]
        public ActionResult KuaförList(FormCollection datafc)
        {
            Class1 model = new Class1();
            List<İl> sehirList = db.İl.OrderBy(f => f.İlAdı).ToList();
            model.SehirList = (from s in sehirList
            ...
            if (p != null)
            {
                values = values.Where(i => i.Adres == p);
